...
export const CompanySection = () => {
    ...
    const [activeTab, setActiveTab] = useState<string>(Object.keys(COMPANIES)[0]);
        ...
        <div className="w-full mt-12 mb-20 animate-in fade-in slide-in-from-bottom-4 duration-700 delay-300">
            ...
            <Tabs defaultValue={companyKeys[0]} value={activeTab} onValueChange={setActiveTab} className="w-full">
                <div className="flex justify-center mb-8 w-full">
                    <TabsList className="bg-muted/30 p-1.5 h-auto w-full md:w-auto flex justify-start md:justify-center overflow-x-auto no-scrollbar scroll-smooth gap-1 rounded-full border border-border/40 backdrop-blur-sm">
                        {companyKeys.map((key) => (
                            <TabsTrigger
                                key={key}
                                value={key}
                            ...
                            >
                                {activeTab === key && (
                                    <motion.div
                                        layoutId="activeTabCompany"
                                        className="absolute inset-0 bg-primary rounded-full shadow-md z-0"
                                        transition={{ type: "spring", bounce: 0.2, duration: 0.6 }}
                                    />
                                )}
                                <span className="relative z-10">{COMPANIES[key].shortName}</span>
                            </TabsTrigger>
                        ))}
                    </TabsList>
                </div>
